...
ERP와 모바일 재고 조회는 같은 거래일의 입고·생산입고를 먼저 반영한 뒤 차감합니다. 서버가 출고를 먼저 반환해도 같은 LOT의 다른 생산일 잔량을 대신 소진하지 않습니다. 관리번호가 없는 과거 거래도 저장된 생산일·원가가 있으면 해당 조건을 지키며, 부족분은 해당 생산분의 음수 재고로 남깁니다. 생산일·원가가 없는 과거 거래는 기존의 넓은 조건으로 조회합니다.

이 변경은 화면의 재고 계산 로직만 수정합니다. 기존 거래·생산일보의 저장 단가, 생산일, 수량은 자동 보정하지 않습니다. 이미 다른 생산분에 잘못 연결해 저장한 재투입은 원래 생산분의 양수 잔량과 잘못 연결된 생산분의 음수 잔량으로 드러날 수 있으며, 해당 기록 정정은 별도로 확인하고 처리해야 합니다. 이 계산 수정에는 추가 서버 SQL/업무 데이터 마이그레이션이 필요하지 않습니다.

`tools/test-production-row-stock.cjs`와 `tools/test-mobile-stock-rows.cjs`에 8/18 생산 290.34kg·출고 290kg, 9/3 생산·출고 각 7.8kg의 가상 사례를 추가했습니다. 입력 순서 24가지, 생산일/원가 불일치와 부족분, 재투입, 이미 저장된 잘못된 연결의 보존을 검사합니다. ERP 테스트는 날짜별 조회와 미래 입고의 소급 차감 방지도 검증합니다.
