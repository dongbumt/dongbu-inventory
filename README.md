...
ERP의 `모바일 관리자` 메뉴에서 계정을 등록하거나 사용 중지할 수 있습니다. 로그인 세션은 해당 브라우저에 저장되어 자동로그인되며, 일정관리·거래내역·생산일보·재고현황·배송기사근태·직원정보·지출관리를 조회할 수 있습니다. 모바일 화면은 읽기 전용이므로 기존 ERP의 데이터 저장 방식에는 영향을 주지 않습니다.
