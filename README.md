...
PC에서는 ERP 메뉴와 동일하게 A4 미리보기를 함께 표시하고, 모바일에서는 입력과 이력 확인에 집중할 수 있도록 미리보기를 숨깁니다. 공용 화면은 전체 ERP 데이터를 조회하지 않으며 냉동창고 요청 이력과 냉동창고명·팩스번호만 서버 함수에서 제한적으로 제공합니다. 요청 품목의 출고·이체처와 품명은 목록 없이 직접 입력합니다. `supabase/schema-rpc-18-cold-storage-public.sql`은 브라우저 사용자가 아닌 Supabase 서버 함수 역할에만 필요한 테이블 권한을 부여합니다.
