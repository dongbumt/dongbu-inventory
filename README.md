...
**부자재 관리 → 부자재 재고현황**은 기본으로 **LOT별 재고**를 표시합니다. 입고일·품목·규격·입고 LOT·거래처와 함께 **입고수량 / 사용수량 / 현재고 / 단위 / 상태**를 확인합니다. 품목명·고유번호·규격·LOT·거래처·비고로 검색하고, 전체 / 재고 있음 / 소진 / 음수 재고로 필터링할 수 있습니다. 아래 입고 LOT 목록의 조회 월과 관계없이 이전 달에 입고된 LOT도 포함합니다.

현재고는 해당 입고 건에 연결된 생산일보 부자재 사용수량을 차감합니다. 사용내역 수정·삭제 후 다시 집계되며, 이름이나 LOT 표시가 같더라도 다른 입고 건의 사용량을 합치지 않습니다. 품목 마스터가 삭제되어도 남아 있는 입고 건은 표시합니다. **품목별 재고**를 선택하면 기존 실사 반영 관리 재고와 실사삭제 기능을 그대로 사용할 수 있습니다. 실사는 품목 단위로 저장되므로 그 차이를 LOT에 임의로 배분하지 않으며, LOT 합계와 실사 반영 관리 재고가 다를 수 있습니다. 기존 업무 데이터·저장 방식·메뉴 권한은 변경하지 않습니다.

검증: `node tools/test-submaterial-lot-stock.cjs` (Playwright / Chrome 필요, `--unit-only`로 계산 검증만 실행). 가상 자료로 LOT 연결, 소수점·소진·음수 재고, 사용량 수정·삭제, 실사 집계 보존, 검색·화면 전환을 확인하며 서버나 실제 업무 데이터에 접근하지 않습니다.
