...
배송기사 전용 화면의 **차량 온도기록지 전송**에서 사진 촬영 또는 사진 선택 후 네 모서리를 확인하고 기록일·차량번호를 입력해 전송합니다. 종이가 밝고 배경이 어두우면 영역을 자동 제안하며, 인식이 어려운 사진은 터치로 모서리를 직접 조정할 수 있습니다. 글자를 OCR로 재작성하지 않고 원본 기록지 영역을 원근 보정하여 사용합니다.
