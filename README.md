...
ERP **생산관리 → 생산일보**에서 작업일자로 조회합니다. 메뉴 진입·조회 시 전송 내역을 서버에서 갱신합니다. 전송된 일보의 **수정**을 누르면 기존 투입 원료와 생산품목·중량은 고정되고, **+ 투입 품목 추가**로 추가 원료를 입력·수정할 수 있습니다. 추가 원료 금액에 따라 생산단가를 다시 계산합니다. 부자재는 기존 **부자재** 버튼으로 수정합니다.
...
서버 설치: 기존 라벨 PIN 및 M02 생산일보 권한 설정 이후 `supabase/schema-rpc-36-label-production-completion.sql`. 일반 생산일보 저장 권한은 그대로 유지하며 라벨 PIN으로 임의의 ERP 거래를 저장할 수 없습니다. 새 출력이력은 서버 작업지시와 대조하고, 여러 브라우저의 이력을 병합하여 오래된 화면이 다른 출력이력을 덮어쓰지 않도록 합니다.
