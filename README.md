...
라벨 인쇄용 창은 ERP와 라벨전용 화면이 공통으로 사용합니다. **70×100mm 또는 100×100mm**, 페이지 여백 0, 라벨 1장당 1페이지를 지정하며 인증 마크와 글꼴을 읽은 뒤 인쇄창을 엽니다. 인쇄용 창의 설정 안내·버튼은 종이에 인쇄되지 않습니다. 기존 라벨 서식과 출력이력/생산량 계산은 변경하지 않습니다.

라벨 위에 날짜·“라벨 출력”이 찍히거나 큰 여백 때문에 아래가 잘리면 브라우저 인쇄창의 **머리글/바닥글을 해제**, **여백 없음**, **배율 100%**, **한 면에 1페이지**, 프린터 용지를 실제 라벨 크기로 설정하세요. 프린터 드라이버의 용지 크기·인쇄 방향·위치 보정도 확인해야 합니다. 웹페이지는 사용자가 저장한 인쇄 설정이나 프린터의 인쇄 불가능 영역을 강제로 변경할 수 없습니다. 한 장만 시험 출력하세요. 같은 인쇄용 창에서 ‘인쇄창 다시 열기’를 누르면 출력이력을 새로 저장하거나 생산량을 더하지 않고 동일 내용으로 다시 인쇄합니다.
