...
완료 후에는 새 라벨 출력·라벨 삭제가 제한되며, 출력이력의 **재출력**은 가능합니다. 다시 누르거나 브라우저를 재시작해도 같은 작업은 중복 등록되지 않습니다. ERP에서 전송된 일보를 삭제한 경우에도 자동 재등록하지 않습니다. 전송 전에 작업지시를 변경해 출력 당시 투입정보와 달라진 경우에는 확인을 요청합니다. 기존 출력이력·일반 생산일보를 자동으로 전송하거나 변경하지 않습니다.

서버 설치: 기존 라벨 PIN 및 M02 생산일보 권한 설정 이후 `supabase/schema-rpc-36-label-production-completion.sql`, `supabase/schema-rpc-37-label-production-full-edit.sql` 순서로 적용합니다. 36이 이미 설치된 서버는 37만 적용해도 됩니다. 일반 생산일보 저장 권한은 그대로 유지하며 라벨 PIN으로 임의의 ERP 거래를 저장할 수 없습니다. 새 출력이력은 서버 작업지시와 대조하고, 여러 브라우저의 이력을 병합하여 오래된 화면이 다른 출력이력을 덮어쓰지 않도록 합니다. 수정하더라도 동일 작업지시의 중복 전송은 계속 차단합니다.
