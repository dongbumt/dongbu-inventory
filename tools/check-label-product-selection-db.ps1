...
try {
  ...
  $schema = Get-Content -LiteralPath (Join-Path $taskRoot 'supabase/schema-rpc-42-label-product-selection.sql') -Encoding UTF8 -Raw
  $migration = Get-Content -LiteralPath (Join-Path $taskRoot 'supabase/migrations/20260915100000_label_product_selection.sql') -Encoding UTF8 -Raw
  if($schema.Replace("`r`n","`n").Trim() -cne $migration.Replace("`r`n","`n").Trim()){ throw 'Product selection schema and migration differ.' }
  $taskArguments = @((Join-Path $PSScriptRoot 'check-label-product-selection-db.py'))
  if($Apply){ $taskArguments += '--apply' }
  $previousEncoding = $OutputEncoding
  try {
    $OutputEncoding = [System.Text.UTF8Encoding]::new($false)
    $schema | python @taskArguments
    if($LASTEXITCODE -ne 0){ throw 'Product selection verification failed; all changes were rolled back.' }
