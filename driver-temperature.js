(function(root){
  'use strict';
  let options,source=null,corners=null,cropped=null,pending=null,busy=false,epoch=0,session='',drag=-1,listEpoch=0;
  ...
  function clearPhoto(){
    epoch++;source=null;corners=null;cropped=null;pending=null;drag=-1;
    ...
    el('temp-camera-input').value='';el('temp-photo-input').value='';el('temp-note').value='';setBusy(false);
  }
  ...
  async function crop(){
    ...
    try{
      ...
      message(result.width<450?'사진 해상도가 낮습니다. 글자가 흐리면 더 가까이 다시 촬영해주세요.':'기록지와 차량번호를 확인한 뒤 전송해주세요.');
  ...
  async function upload(){
    ...
    if(!pending){
      ...
      if(!date||!vehicle){message('기록일과 차량번호를 입력해주세요.',true);return;}
  ...
  async function loadHistory(){
    ...
    try{
      ...
      if(!el('temp-vehicle').value)el('temp-vehicle').value=rows[0].vehicle_no||'';
