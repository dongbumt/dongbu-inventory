-- Product availability affects new selections only; historical records are never rewritten.
-- Missing isActive is active. Older clients preserve an existing inactive value.
...
begin
  ...
  if exists(
    ...
    where k not in (
      ...
      'packunit','itemno','grade','isActive'
    )
  ) then
  ...
  if p_record ? 'isActive' and jsonb_typeof(p_record->'isActive') <> 'boolean' then
    raise exception '사용 여부는 사용 또는 사용안함으로 지정해주세요.';
  end if;
  ...
  v_record := jsonb_build_object(
    'id',v_id,
    'isActive',case when p_record ? 'isActive' then (p_record->>'isActive')::boolean
                    else coalesce(v_before->'isActive' <> 'false'::jsonb, true) end,
