-- Extend row-level stock tracking to new ordinary inbound records.
-- Historical rows are deliberately left untouched: only a row which already
-- carries stockRowId participates in exact source allocation.
do $dbmt$ begin
...
begin
  ...
  -- later row references it, its identity/cost may not be silently changed or
    ...
    if v_previous_source_id<>'' then
      ...
      if v_has_links then
        ...
        if new.product is distinct from old.product or new.lot is distinct from old.lot
          ...
          or coalesce(new.stock_location,'가공장') is distinct from coalesce(old.stock_location,'가공장')
          or round(coalesce(new.price,0))<>round(coalesce(old.price,0)) then
          raise exception '연결된 입고 재고원본의 품목·LOT·원산지·규격·보관장소·단가는 변경할 수 없습니다.';
        end if;
