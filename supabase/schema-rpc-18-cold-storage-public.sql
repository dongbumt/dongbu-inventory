...
begin
  ...
  values ('coldStorageRequests', '[]'::jsonb, now())
  on conflict (key) do nothing;

  select case when jsonb_typeof(payload) = 'array' then payload else '[]'::jsonb end
    into requests
  from public.app_data
  where key = 'coldStorageRequests'
  for update;

  select exists(
    select 1 from jsonb_array_elements(requests) elem where elem->>'id' = record_id
  ) into existed;

  if p_action = 'save' then
    target_record := p_record;
    requests := jsonb_build_array(p_record) || coalesce((
      select jsonb_agg(elem)
      from jsonb_array_elements(requests) elem
      where elem->>'id' <> record_id
    ), '[]'::jsonb);
    log_action := case when existed then '수정' else '저장' end;
  else
    select elem into target_record
    from jsonb_array_elements(requests) elem
    where elem->>'id' = record_id
    limit 1;
    requests := coalesce((
  ...
  set payload = requests, updated_at = now()
  where key = 'coldStorageRequests';

  if target_record is not null then
    requester_name := case when target_record->>'requesterId' = 'dongbu_distribution'
      then '(주)동부축산유통' else '주식회사 동부엠티' end;
    ...
    on conflict (key) do nothing;
    select case when jsonb_typeof(payload) = 'array' then payload else '[]'::jsonb end
      into logs from public.app_data where key = 'dataChangeLogs' for update;
    update public.app_data
    set payload = jsonb_build_array(log_entry) || logs, updated_at = now()
